    ...
    <nav className="fixed top-0 w-full z-50 transition-all duration-300 bg-slate-950/80 backdrop-blur-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-14 sm:h-16 md:h-20">
          ...
          <div className="hidden md:flex items-center space-x-6 lg:space-x-8">
            {user ? (
              <>
                <NavLink to="/dashboard" className="text-gray-300 hover:text-white text-sm lg:text-base">{t('nav.dashboard')}</NavLink>
                <NavLink to="/profile" className="text-gray-300 hover:text-white text-sm lg:text-base">{t('nav.profile')}</NavLink>
                <button onClick={handleLogout} className="text-gray-300 hover:text-white text-sm lg:text-base">{t('nav.logOut')}</button>
              </>
            ) : (
              <>
      ...
      {mobileMenuIsOpen && (
        <div className="px-4 py-4 space-y-3 md:hidden bg-slate-900/95 backdrop-blur-lg border-t border-slate-800">
          {user ? (
            <>
              <NavLink to="/dashboard" className="block text-center text-gray-300 hover:text-white" onClick={close}>{t('nav.dashboard')}</NavLink>
              <NavLink to="/profile" className="block text-center text-gray-300 hover:text-white" onClick={close}>{t('nav.profile')}</NavLink>
              <button onClick={handleLogout} className="block w-full text-center text-gray-300 hover:text-white">{t('nav.logOut')}</button>
            </>
          ) : (
            <>
